...
**How to apply / verify:** "I don't see the menu item" complaints on this ERP are usually (a) the group is collapsed, (b) the item is buried in a long group below the fold, or (c) a label-name mismatch — not a missing route. Verify from the running UI by logging in with the seeded admin account (credentials documented in `replit.md`) and expanding the group; restart the `artifacts/erp: web` workflow first so the canvas iframe serves the fresh build (a stale iframe is a common false "missing" report).
