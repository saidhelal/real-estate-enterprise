...
- `pnpm --filter @workspace/api-server run dev` — run the API server (reads `PORT`, default 8080)
...
- `pnpm --filter @workspace/db run migrate` — apply pending migrations (generate them with `drizzle-kit generate`; never `db push`)
...
- Cross-module sales side effects (`lib/integrations.ts`): unit availability is a *derived* projection, never hand-authored. `recomputeUnitStatus(tx, unitId)` resolves `units.unitStatusId` from the strongest live claim (active/draft contract → sold, else active/confirmed reservation → reserved, else available) and writes only on change. Every reservation/contract create/update/delete, reservation→contract convert, unit-transfer, and contract-cancellation recomputes the affected unit(s). `ensureLegalContractForContract(tx, contract)` auto-registers a `legal_contracts` row (idempotent per `sourceModule='sales'+sourceId`) and back-links `contracts.legalContractId` on contract create/convert — the canonical contract FKs are never repointed. Contract-cancellation flips the contract to `cancelled`, reverses its GL, and frees the unit; unit-transfer repoints `contracts.unitId` and refreshes both ends. Lifecycle-log POSTs (cancellations, transfers) mutate+validate the canonical contract first and 404 before inserting their log row, so no orphan records are created.
