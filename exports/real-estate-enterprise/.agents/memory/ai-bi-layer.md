...
## Shared system prompt: chat vs strict-JSON analysis
- `BASE_SYSTEM` (routes/ai.ts) is reused by BOTH the chat SSE path AND the 8 strict-JSON `runAnalysis` endpoints. Any chat-only formatting added to it (Markdown headings, `erp-action` blocks, the "Executive Advisor" briefing) is a conflicting prior for the JSON endpoints. **How to apply:** when adding chat-format guidance to BASE_SYSTEM, include a PRECEDENCE clause that the strict JSON/fixed schema wins (map the analysis into JSON fields, emit no Markdown/action blocks), or split into separate chat vs analysis system prompts. The JSON parse path already has a 502 failure mode, so malformed output is not silent but is user-visible.
